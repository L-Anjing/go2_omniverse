...
2. 指令课程改为三档渐进扩展（±0.5→±0.75@30k→±1.0@55k→±2.0@90k），消除两次大幅扩档导致的策略崩溃
3. 地形课程`move_down`误改为比例阈值（cmd_vel×25s×50%≈6.25m），导致转弯指令下net位移不足判为失败，地形归零、策略收敛至"双后腿悬空"轮椅局部最优——已回退为固定1.5m阈值（已验证稳定）